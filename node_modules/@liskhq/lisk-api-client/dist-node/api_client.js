...
var getClientHeaders = function getClientHeaders(clientOptions) {
	if (!clientOptions) {
		return {};
	}

	var _clientOptions$name = clientOptions.name,
	    name = _clientOptions$name === undefined ? '????' : _clientOptions$name,
	    _clientOptions$versio = clientOptions.version,
	    version = _clientOptions$versio === undefined ? '????' : _clientOptions$versio,
	    _clientOptions$engine = clientOptions.engine,
	    engine = _clientOptions$engine === undefined ? '????' : _clientOptions$engine;
...
var APIClient = function () {
	...
	(0, _createClass3.default)(APIClient, [{
		...
		value: function initialize(nodes) {
			...
			this.headers = (0, _assign2.default)({}, commonHeaders, options.nethash ? { nethash: options.nethash } : {}, getClientHeaders(options.client));
